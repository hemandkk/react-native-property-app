...
- [Discord community](https://chat.expo.dev): Chat with Expo users and ask
  questions. Rules to avoid this later

For Expo + committed native folders:

1. After plugin changes → regenerate native

Any time you:

add/remove Expo plugin upgrade Expo SDK change app.json change
expo-build-properties add Clerk/Firebase/etc plugin
...
git add android ios git commit

This is the biggest lesson.

2. Before EAS build, verify config

Run:

npx expo config --type public

This catches plugin/config issues early.

3. Keep EAS CLI updated
...
Old CLI versions cause confusing build behavior.

4. Use build:inspect when debugging

Very useful:

eas build:inspect --platform android --stage archive --output inspect
